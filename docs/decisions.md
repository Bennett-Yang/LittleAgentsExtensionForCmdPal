...
| D-CLIP | Clipboard reads handle text, image-only, and empty clipboard payloads from the Command Palette host. | `LittleAgentsExtension/Llm/ClipboardReader.cs`, `LittleAgentsExtension/NativeMethods.txt`, `LittleAgentsExtension.Tests/ClipboardReaderTests.cs`, `.omo/evidence/spike-clipboard-mta.md` |
...
| D-ERR | Network, TLS, status-code, and generic provider errors are surfaced without exposing secrets. | `LittleAgentsExtension/Pages/Run/ChatRunPage.cs`, `LittleAgentsExtension.Tests/ChatRunPageStreamingTests.cs` |
| D-BASEURL | Provider base URLs must be absolute HTTP(S), include the API root, and are documented with examples. | `LittleAgentsExtension/Pages/Providers/ProviderEditFormPage.cs`, `LittleAgentsExtension.Tests/ProviderEditFormTests.cs`, `README.md` |
